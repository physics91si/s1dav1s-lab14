...
lab13dir = os.path.join(SUBMISSIONS, user, 'lab13')
...
for target in targets:
	...
	# hg update to show files, if necessary
		if not target in os.listdir('.'):
			ans = raw_input("%s not found in %s. Try hg update? \nWarning: this will overwrite current directory contents. [y/n] " % (target, lab13dir))
			if 'y' in ans:
				shell("hg update tip")
		if not target in os.listdir('.'):
	       		raise IOError
	except:
		print "Error: unable to find %s" % os.path.join(lab13dir, target)
		continue
...
# Copy over and commit
	try:
		...
		shell("cp %s ." % os.path.join(lab13dir, target))
		shell("hg add %s" % target)
		print "%s copied from %s and added to repo" % (target, lab13dir)
	except:
